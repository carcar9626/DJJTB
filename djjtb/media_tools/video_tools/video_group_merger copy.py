...
def build_crop_filter(crop_aspect, width, height):
    ...
    if crop_aspect == '1':  # 16:9 horizontal
        ...
        if target_w <= width:
            ...
            return f"crop={width}:{target_h}:0:{offset_y}"
    else:  # 9:16 vertical
...
def simple_merge_all_videos(videos, output_dir):
    ...
    if sizing_method.startswith('first_video'):
        ...
        # For crop mode, we'll determine dimensions after cropping
        target_width, target_height = 1920, 1080  # Default, will be adjusted
        ...
        if sizing_method in ['first_video_blur', 'first_video_pad', 'fixed_1920x1080_blur', 'fixed_1920x1080_pad', 'crop']:
            ...
            for i, video in enumerate(videos):
                ...
                if sizing_method == 'crop':
                    # Crop to fit aspect ratio, then scale
                    crop_filter = build_crop_filter(crop_aspect, curr_width, curr_height)
                    process_cmd = [
                        "ffmpeg", "-y", "-i", video,
...
def main():
    while True:
        ...
        print("\033[1;33mVideo Merger\033[0m")
