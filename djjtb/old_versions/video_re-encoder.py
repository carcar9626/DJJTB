...
def reencode_videos(input_path, subfolders=False, audio_choice='1'):
    ...
    for i, video_path in enumerate(videos, 1):
        ...
        sys.stdout.write(f"\033[93m\rProcessing \033[0m{i}/{total_videos} ({progress:.1f}%)...")
        sys.stdout.flush()

        if codec == "copy":
            cmd = ["ffmpeg", "-i", str(video_path), "-c", "copy", "-y", output]
        else:
        ...
        try:
            ...
            logger.info(f"\033[93mRe-encoded\033[0m {filename} \033[93m to\033[0m {output}")
            successful_videos += 1
        except subprocess.CalledProcessError as e:
            logger.error(f"\033[93mError re-encoding\033[0m {filename}: {e.stderr}")
            print(f"\n\033[93mError re-encoding {filename}: {e.stderr}\033[0m")
    ...
    logger.info(f"\033[93mRe-encoded\033[0m {successful_videos} \033[93mof\033[0m {total_videos} \033[93mvideos successfully\033[0m")
    print("\n\033[93mRe-encoding Summary\033[0m")
    print("-------------------")
    print(f"\033[93mVideos processed:\033[0m {total_videos}")
    print(f"\033[93mVideos successfully re-encoded:\033[0m {successful_videos}")
    print(f"\033[93mOutput folder(s):\033[0m {', '.join(output_base_dirs)}")
    print()
...
def main():
    while True:
        ...
        print("\033[1;93mVideo Re-Encoder\033[0m")
