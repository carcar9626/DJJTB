...
def resize_images(images, src_path, dimension_type, desired_width, desired_height, output_format, manual_mode='1'):
    """Resize images proportionally based on desired width or height."""
    # Set output directory
    output_base = os.path.join(src_path, "Output", "Resized")
    os.makedirs(output_base, exist_ok=True)

    log_dir = os.path.join(src_path, "Output")
        ...
        try:
            with Image.open(img_path) as img:
                ...
                if orig_width <= 0 or orig_height <= 0:
                    raise ValueError(f"Invalid source dimensions: {orig_width}x{orig_height}")

                if dimension_type == 1:  # Width (scale up or down to match desired width)
                    if orig_width == 0:
                        raise ValueError("Source image has zero width")
                    target_width = desired_width
                    target_height = max(1, int(orig_height * (desired_width / orig_width)))
                elif dimension_type == 2:  # Height (scale up or down to match desired height)
                    if orig_height == 0:
                ...
                elif dimension_type == 3:  # Longest Edge (scale up or down so longest side matches)
                ...
                elif dimension_type == 4:  # Manual Dimensions
                    target_width = desired_width
                    target_height = desired_height
                ...
                if dimension_type == 4 and manual_mode == '2':  # Pad mode
                    scale = min(target_width / orig_width, target_height / orig_height)
                    new_w = max(1, int(orig_width * scale))
                    new_h = max(1, int(orig_height * scale))
                    img_scaled = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
                    canvas = Image.new(img.mode, (target_width, target_height), (255, 255, 255) if img.mode == 'RGB' else (255,))
...
if __name__ == '__main__':
    while True:
        ...
        manual_mode = '1'
        if dimension_type != 4:
            desired_width = djj.get_int_input(
            ...
            print()
            # For manual dimensions, get both width and height once
            desired_width = djj.get_int_input(
                "Enter desired width in px",
                min_val=1
            )
            print()
            desired_height = djj.get_int_input(
                "Enter desired height in px",
                min_val=1
            )
        print()
        ...
        successful, failed, output_base = resize_images(
            images, src_path, dimension_type, desired_width, desired_height, output_format, manual_mode
        )
