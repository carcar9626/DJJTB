...
def get_crop_edges():
    ...
    """
    selected = set()
    all_keys = {key for key, _in CROP_EDGE_OPTIONS}
    while True:
...
def rotate_or_flip_images(images, operation, choice, custom_angle, output_format):
    """Rotate or flip images based on user input."""
    # Determine the output directory for rotated/flipped images
    first_image_dir = os.path.dirname(images[0])
    output_dir = os.path.join(first_image_dir, "Output", "RotatedFlipped")
    os.makedirs(output_dir, exist_ok=True)
    logger = setup_logging(output_dir)
    ...
    for i, img_path in enumerate(images, 1):
        try:
            with Image.open(img_path) as img:
                ...
                # Save in source format — output sits in each image's own parent/Output/RotatedFlipped/
                img_path_obj = pathlib.Path(img_path)
                img_output_dir = img_path_obj.parent / "Output" / "RotatedFlipped"
                img_output_dir.mkdir(parents=True, exist_ok=True)
                output_filename = f"{img_path_obj.stem}_rf{file_extension}"
                output_path = img_output_dir / output_filename
                if output_path.exists():
                    skipped.append(img_path_obj.name)
                    sys.stdout.write(f"\rProcessing {i}/{len(images)} ({i/len(images)*100:.1f}%)...")
                ...
                img.save(str(output_path), format=pillow_format, quality=95 if pillow_format == 'JPEG' else None)
                successful.append(img_path_obj.name)
                sys.stdout.write(f"\rProcessing {i}/{len(images)} ({i/len(images)*100:.1f}%)...")
                sys.stdout.flush()
        except Exception as e:
...
def main():
    while True:
        ...
        print()
        # ── Operation: Pad vs Crop vs Crop+Resize vs Resize only vs Rotate/Flip ─────────────
