...
def copy_source_files(source_files, output_path):
    ...
    for source_file in source_files:
        try:
            ...
            destination_path = pathlib.Path(output_path) / source_path.name
    ...
    if copied_count > 0:
        print(f"\033[93m📁 Copied\033[0m {copied_count} \033[93msource file(s) to output directory\033[0m")
...
def process_face_swap(mode, source_files, target_files, output_path, add_suffix, tag_source, use_enhanced_mode):
    ...
    # Copy source files to output directory if processing was successful
    if success_count > 0:
