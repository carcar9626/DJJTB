...
### script runs from top to bottom with outputs processed, but no swapping was done, just copied the original
...
# ============================================================================
# 🎚️ DEFAULT FACE ALIGNMENT CONFIGURATION
# ============================================================================
# These are starting defaults - you'll be prompted to adjust during runtime

# Face mask extends the swap area (CRITICAL for chin issues!)
# Higher = more coverage of chin/jaw area + better source likeness
# Range: 0-100 | Default: 40 | Try 50-60 if getting chin shadows
DEFAULT_FACE_MASK_PADDING = 40

# Smooths the edges where face meets background
# Higher = softer blend (helps hide seams)
# Range: 0.0-1.0 | Default: 0.4 | Try 0.5-0.6 for more blending
DEFAULT_FACE_MASK_BLUR = 0.4
...
def build_facefusion_args(face_mask_padding, face_mask_blur):
    """Build FaceFusion arguments from configuration"""
    args = []
    
    # CRITICAL: Tell FaceFusion to actually DO the face swap!
    args.extend(["--processors", "face_swapper"])
...
def get_face_mask_settings():
    """Prompt user for face mask padding and blur settings"""
    print("\033[1;93m🎚️  Face Mask Settings (Chin Coverage & Blending)\033[0m")
    print()
    
    # Face mask padding
    print("\033[93mFace Mask Padding\033[0m (controls chin/jaw coverage)")
    print("  • Higher values = more chin coverage + better source likeness")
    print("  • Lower values = more natural blend but may show original chin")
    print("  • Recommended range: 40-60 for most cases")
    print()
    
    padding_input = input(f"\033[93mEnter padding value (0-100)\033[0m [default: {DEFAULT_FACE_MASK_PADDING}]: ").strip()
    
    if padding_input == '':
        face_mask_padding = DEFAULT_FACE_MASK_PADDING
    else:
        try:
            face_mask_padding = int(padding_input)
            if face_mask_padding < 0 or face_mask_padding > 100:
                print(f"\033[93m⚠️  Value out of range, using default: {DEFAULT_FACE_MASK_PADDING}\033[0m")
                face_mask_padding = DEFAULT_FACE_MASK_PADDING
        except ValueError:
            print(f"\033[93m⚠️  Invalid input, using default: {DEFAULT_FACE_MASK_PADDING}\033[0m")
            face_mask_padding = DEFAULT_FACE_MASK_PADDING
    
    print()
    
    # Face mask blur
    print("\033[93mFace Mask Blur\033[0m (smooths blend edges)")
    print("  • Higher values = softer, more gradual blending")
    print("  • Lower values = sharper edges (may show seams)")
    print("  • Recommended range: 0.3-0.6")
    print()
    
    blur_input = input(f"\033[93mEnter blur value (0.0-1.0)\033[0m [default: {DEFAULT_FACE_MASK_BLUR}]: ").strip()
    
    if blur_input == '':
        face_mask_blur = DEFAULT_FACE_MASK_BLUR
    else:
        try:
            face_mask_blur = float(blur_input)
            if face_mask_blur < 0.0 or face_mask_blur > 1.0:
                print(f"\033[93m⚠️  Value out of range, using default: {DEFAULT_FACE_MASK_BLUR}\033[0m")
                face_mask_blur = DEFAULT_FACE_MASK_BLUR
        except ValueError:
            print(f"\033[93m⚠️  Invalid input, using default: {DEFAULT_FACE_MASK_BLUR}\033[0m")
            face_mask_blur = DEFAULT_FACE_MASK_BLUR
    
    print()
    return face_mask_padding, face_mask_blur
...
def process_single_headless(source_file, target_file, output_file, face_mask_padding, face_mask_blur, use_enhanced_mode=False):
    ...
    cmd.extend(build_facefusion_args(face_mask_padding, face_mask_blur))
...
def process_batch_job(source_file, target_files, output_path, face_mask_padding, face_mask_blur, add_suffix=True, use_enhanced_mode=False):
    ...
    config_args = build_facefusion_args(face_mask_padding, face_mask_blur)
...
def process_face_swap(mode, source_files, target_files, output_path, add_suffix, tag_source, target_action, face_mask_padding, face_mask_blur, use_enhanced_mode):
    ...
    if mode == '2':
        ...
        success, error_msg = process_single_headless(source_file, target_file, output_file, face_mask_padding, face_mask_blur, use_enhanced_mode)
        ...
        source_file = source_files[0]
        success_count, error_count, error_messages = process_batch_job(source_file, target_files, output_path, face_mask_padding, face_mask_blur, add_suffix, use_enhanced_mode)
...
def main():
    ...
    while True:
        ...
        print()
        # Get face mask settings from user
        face_mask_padding, face_mask_blur = get_face_mask_settings()
        
        # UPDATE GLOBALS with user's choices
        global FACE_MASK_PADDING, FACE_MASK_BLUR
        FACE_MASK_PADDING = face_mask_padding
        FACE_MASK_BLUR = face_mask_blur
        
        # Get face mask settings
        face_mask_padding, face_mask_blur = get_face_mask_settings()
        ...
        process_face_swap(mode, source_files, target_files, output_path, add_suffix, tag_source, target_action, face_mask_padding, face_mask_blur, use_enhanced_mode)
