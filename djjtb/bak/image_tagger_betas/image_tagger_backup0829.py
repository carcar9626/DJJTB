...
def process_folder_mode(images: List[str], csv_confidence, model, processor, output_dir: str,
    ...
    for csv_round, (csv_file, confidence_threshold) in enumerate(csv_confidence, 1):
        ...
        for batch_idx in range(total_batches):
            ...
            sys.stdout.write(f"\r  \033[93mBatch\033[0m {batch_idx + 1}\033[93m/\033[0m{total_batches} ({progress}%) - {processed_count}\033[93m/\033[0m{len(images)} images...\033[0m")
            if batch_nsfw > 0:
...
def main():
    while True:
        ...
        if folder_mode:
            ...
            # Here you can insert per-CSV confidence handling (dict or interactive prompt)
            # Example: override confidence per CSV
            csv_confidence = {
                "positions_advance.csv": 0.8,
                "positions_basic.csv": 0.3,
                "top_clothing.csv": 0.4,
                "color.csv": 0.3,
                "bottom_clothing.csv": 0.4,
                "barefoot.csv": 0.8,
                "clothing_material.csv": 0.6,
                "feet_only.csv": 0.4,
                "glasses_only.csv": 0.8,
                "nsfw_actions.csv": 0.8,
                "nudity_only.csv": 0.8,
                "top_pattern.csv": 0.6,
                "settings.csv": 0.4,
                "hair_color.csv": 0.6,
        ...
        if folder_mode:
            ...
            # Single CSV processing (original logic) with timing
