...
def process_folder_mode(images: List[str], csv_files: List[str], model, processor, output_dir: str,
    ...
    for csv_round, csv_file in enumerate(csv_files, 1):
        ...
        for batch_idx in range(total_batches):
            ...
            sys.stdout.write(f"\r  \033[93mBatch\033[0m {batch_idx + 1}\033[93m/\033[0m{total_batches} ({progress}%) - {processed_count}\033[93m/\033[0m{len(images)} images... \033[36m[Round: {format_elapsed_time(round_elapsed)}, Total: {format_elapsed_time(total_elapsed)}]\033[0m")
            if batch_nsfw > 0:
...
def main():
    while True:
        ...
        if folder_mode:
            ...
            # Single CSV processing with timing
