...
def load_tag_queries_from_csv(csv_path: str) -> Dict:
    ...
    try:
        ...
        print(f"  \033[93m Loaded:\033[0m\n       \033[93m{total_categories}\033[0m categories \033[93m{total_subcategories}\033[0m subcategories\n  Total Queries: \033[93m{total_queries}\033[0m")
...
def clean_tag_name(query: str, category: str, subcategory: str) -> str:
    ...
    if cleaned.startswith(" photography"):
        cleaned = cleaned[12:]
    ...
    if cleaned.startswith("top "):
        cleaned = cleaned[4:]
        
    if cleaned.startswith(" setting"):
        cleaned = cleaned[8:]
...
def read_existing_xmp_data(xmp_path):
    """Read existing tags AND face regions from XMP file if it exists."""
    if not os.path.exists(xmp_path):
        return {'tags': [], 'face_regions': {}}
    ...
    try:
        ...
        # Extract existing tags (any format)
        existing_tags = []
        for rdf_li in desc.findall('.//rdf:li', namespaces):
            if rdf_li.text and rdf_li.text.strip():
                existing_tags.append(rdf_li.text.strip())
        
        # Extract face regions (both formats)
        face_regions = {}
        
        # Microsoft Photo format
        mp_region = desc.find('MP:RegionInfo', namespaces)
        if mp_region is not None:
            face_regions['mp_region'] = mp_region
        
        # MWG format
        mwg_region = desc.find('mwg-rs:Regions', namespaces)
        if mwg_region is not None:
            face_regions['mwg_region'] = mwg_region
        ...
        print(f"\033[33mWarning: Error reading XMP {xmp_path}: {e}\033[0m")
        return {'tags': [], 'face_regions': {}}
...
def export_xmp_sidecar_files(db_path: str, merge_mode: bool, include_nsfw: bool, logger):
    ...
    for file_path, data in images_dict.items():
        ...
        if tags:
            ...
            # Handle merge mode
            if merge_mode:
                existing_tags = read_existing_xmp_data(xmp_path)
                # Combine and deduplicate
                merge_tags(existing_data['tags'], new_tags)
                face_regions = existing_data['face_regions']
            else:
                all_tags = tags
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
    <rdf:Description rdf:about="" xmlns:dc="http://purl.org/dc/elements/1.1/">
      ...
      <!-- ADD: Face region blocks if they exist -->
      <!-- ADD: Multiple tag formats (digiKam, Lightroom, etc.) -->
    </rdf:Description>
  </rdf:RDF>
</x:xmpmeta>'''
...
def process_folder_mode(images: List[str], csv_confidence, model, processor, output_dir: str,
                       ...
                       export_xmp: bool, merge_tags: bool, logger):
    ...
    for csv_round, (csv_file, confidence_threshold) in enumerate(csv_confidence, 1):
        ...
        for batch_idx in range(total_batches):
            ...
            sys.stdout.write(f"\r \033[93mBatch\033[0m {batch_idx + 1}\033[93m/\033[0m{total_batches} ({progress}%) - {processed_count}\033[93m/\033[0m{len(images)} images... \n\033[36m[Round: {format_elapsed_time(round_elapsed)}, Total: {format_elapsed_time(total_elapsed)}]\033[0m")
        ...
        print(f"\n  \033[32m✅Round {csv_round} complete:\033[0m {len(images)} images processed")
...
def main():
    while True:
        ...
        if folder_mode:
            ...
            confidence_input = input("\033[93mDefault confidence threshold for unlisted CSVs [0.1-0.9, default: 0.15]:\n\033[0m -> ").strip()
            try:
            ...
            confidence_presets = {
                ...
                "clothing_outer.csv": 0.85,
                "clothing_patterns.csv": 0.6,
                "clothing_shoes.csv": 0.8,
                ...
                "nsfw.csv": 0.4,
                "nudity.csv": 0.7,
                "positions_advance.csv": 0.5,
                "positions_basic.csv": 0.45,
                "settings.csv": 0.5
            }
        ...
        if folder_mode:
            ...
            for batch_idx in range(total_batches):
                ...
                sys.stdout.write(f"\r\033[93mProcessing batch\033[0m {batch_idx + 1}/{total_batches} ({progress}%) - {processed_count}/{len(images)} \033[93mimages...\033[0m \033[36m[Elapsed: {format_elapsed_time(current_elapsed)}]\033[0m")
                if batch_nsfw > 0:
                    sys.stdout.write(f" [\033[31m{batch_nsfw} NSFW\033[0m]")
                print()
                sys.stdout.flush()
        ...
        if export_xmp:
            ...
            xmp_count, nsfw_count = export_xmp_sidecar_files(db_path, merge_tags, include_nsfw, logger)
