...
def clean_tag_name(query: str, category: str) -> str:
    """Clean up tag names by removing redundant phrases."""
    ...
    if cleaned.startswith("wearing ") and category == "clothing":
        cleaned = cleaned[8:]
        
    # For non-clothing, keep "wearing" context when it adds meaning
    elif "wearing" in cleaned and category != "clothing":
        # Keep it - like "person wearing glasses" -> "wearing glasses"
        pass
    
    # Remove "in" for styles/settings when redundant
    if cleaned.startswith("in ") and category in ["clothing", "location"]:
        cleaned = cleaned[3:]
    
    # Clean up extra spaces and capitalize properly
    cleaned = " ".join(cleaned.split())  # Remove extra spaces
    cleaned = cleaned.title()  # Proper title case
    
    # Handle special cases
    if cleaned.lower() in ["nsfw", "topless", "nude", "nudity"]:
