...
def main():
    while True:  # Outer loop for restarting with new path
        ...
        input_path = input("\033[33m📁 Enter folder path:\n -> \033[0m").strip()
        input_path_obj = pathlib.Path(input_path).resolve()
        print()
        if not input_path_obj.exists():
            print(f"\033[33m❌ Error: \033[0m'{input_path}' \033[33mnot found.\033[0m\n")
            continue
        ...
        while True:  # Inner loop for searching with same path
            print()
            search_term = input("\033[33m🔍 Enter search term:\n -> \033[0m").strip()
            if not search_term:
                print("\033[33m❌ No search term entered.\033[0m\n")
                continue

            all_images = get_all_images(input_path_obj, include_sub)
            total = len(all_images)
            if total == 0:
                print("\033[33m❌ No images found.\033[0m")
                break

            print(f"\n\033[33m🔎 Searching\033[0m {total} images for: '{search_term}' ...\n")
            ...
            for i, img_path in enumerate(all_images, 1):
                ...
                print(f"\r\033[33mProcessing images \033[0m{i}/{total} ({int(i/total*100)}%)...", end="", flush=True)
            print("\n✅ Matching complete.\n")
            ...
            do_output = djj.prompt_choice(
                "\033[33mSubfolder results?\033[0m\n1. Yes, 2. No",
                ['1', '2'],
                default='2'
            ) == '1'
            print()
