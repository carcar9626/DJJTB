...
def collect_and_select_files(input_path, include_sub=False):
    ...
    if len(all_files) > 50:
        print(f"\033[33mToo many files ({len(all_files)}) to display individually.\033[0m")
        analyze_all = djj.prompt_choice(
            "\033[33mProceed with all files?\033[0m\n1. Yes, analyze all\n2. No, go back\n",
            ['1', '2'],
            default='1'
        )
...
def main():
    ...
    while True:
        ...
        if not MAGIC_AVAILABLE:
            print("\033[33m⚠️  python-magic not available - using extension-based detection only\033[0m")
        ...
        if not all_files:
            print("❌ \033[33mNo valid files found. Try again.\033[0m\n")
            continue

        print(f"✅ \033[33m{len(all_files)} files selected for analysis\033[0m")
        print()
        ...
        for idx, file_path in enumerate(all_files, 1):
            ...
            sys.stdout.write(f"\r\033[33mAnalyzing\033[0m {idx}\033[33m/\033[0m{len(all_files)}: {display_name}                    ")
            sys.stdout.flush()
        ...
        print()
        print("\033[33m🔍 Enhanced File Analysis Summary\033[0m")
        print("-" * 35)
        ...
        print(f"\033[33mTotal files analyzed:\033[0m {len(files_data)}")
        print()
        
        print("\033[33m🎯 True file types (magic number detection):\033[0m")
        for file_type, count in sorted(true_type_counts.items()):
            print(f"  {file_type}: {count}")
        
        print()
        print("\033[33m📝 Extension-based types:\033[0m")
        for file_type, count in sorted(ext_type_counts.items()):
            print(f"  {file_type}: {count}")
        
        print()
        print("\033[33m🔗 Extensions (top 10):\033[0m")
        ...
        if MAGIC_AVAILABLE:
            print()
            print("\033[33m🧬 MIME types:\033[0m")
        ...
        if mismatched_files:
            ...
            print("\033[33mFiles where extension doesn't match true type:\033[0m")
        ...
        export_csv = djj.prompt_choice(
            "\033[33mExport detailed analysis to CSV?\033[0m\n1. Yes\n2. No\n",
            ['1', '2'],
            default='1'
        ) == '1'
        ...
        if export_csv:
            ...
            if export_to_csv(files_data, csv_path):
                print(f"\033[33m📊 Enhanced analysis exported to:\033[0m\n{csv_path}")
                print()
                ...
                print("\033[33m❌ Failed to export CSV\033[0m")
